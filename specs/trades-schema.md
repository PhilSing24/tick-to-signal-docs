...
|---------|--------|-------------|
| FH sends | 12 | — |
| TP adds | 13 | `tpRecvTimeUtcNs` |
| WDB adds | 14 | `wdbRecvTimeUtcNs` |
| RDB adds | 14 | `rdbRecvTimeUtcNs` |
...
| 14 | `wdbRecvTimeUtcNs` | long | WDB | WDB receive time — **WDB only** |
| 14 | `rdbRecvTimeUtcNs` | long | RDB | RDB receive time — **RDB only** |

**Note:** Field 14 is either `wdbRecvTimeUtcNs` (in WDB) or `rdbRecvTimeUtcNs` (in RDB), not both.
...
- `telemetry_latency_fh` — FH segment percentiles (trade + quote handlers)

This provides both granular debugging and efficient dashboarding.

## Derived Analytics

### MLE (Dollar Imbalance Bars)

The MLE computes information-driven bars from trade data:

```q
/ Tick rule for trade direction
bt = $[price > prevPrice; 1; price < prevPrice; -1; prevBt]

/ Dollar imbalance accumulator
theta += bt * price * qty

/ Emit bar when |theta| >= threshold
```

See ADR-011 for MLE details.

### RTE (VWAP)

The RTE computes VWAP from trade data:
...
Trade data is logged by TP to `logs/YYYY.MM.DD.log` (single file containing all market data) in `-11!` compatible format. On WDB/MLE restart, trades are automatically replayed:

```q
/ Automatic replay on WDB startup
.wdb.replay[.z.D]
```
...
| 2.4 | 2026-01-18 | Clarified WDB vs RDB receive timestamps, added MLE reference |
...
- `adr-011-financial-machine-learning.md` — MLE bars
